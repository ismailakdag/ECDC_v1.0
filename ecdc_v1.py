...
y_s21=dataset.iloc[:,2:3]
y_s31=dataset.iloc[:,3:4]
#Feature scaling.
...
DC_PWM.start(0)
print("The R-square value for Random Forrest Regressor S21: ",r2_score(y_s21_test,y_s21_pred))
print("The R-square value for Random Forrest Regressor S31: ",r2_score(y_s31_test,y_s31_pred))
...
while True:
    ...
    if(data!="exit"):
        ...
        if(volt<=15):            
            ...
            total_pred=str(pred_res_s21)+","+str(pred_res_s31)+","+errmsg
            print(total_pred)
            client_sock.send(total_pred)
        #Check if the voltage exceeds the boundary.And send the error message.
        #FIXME
        elif(volt>15):
            volt=15
            errmsg="The voltage value must be lower than 15 V."
            client_sock.send("-,-,"+errmsg)
    #Normally the data will get from Bluetooth with comma but for closing the connection another function has been written in Android Studio part.
